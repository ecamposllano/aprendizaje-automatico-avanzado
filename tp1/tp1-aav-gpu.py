...
def softmax(u):
    exp_u = cp.exp(u - cp.max(u))
    return exp_u / cp.sum(exp_u)

def propagacion_batch(W, W_prima, batch_c_indices):
    """
    batch_c_indices: Array o lista de shape (B, C) con los índices de contexto del lote.
    """
    B, C = batch_c_indices.shape
    N = W.shape[1]

    # Sumamos los embeddings de contexto para todo el batch de golpe: shape (B, N)
    # W[batch_c_indices] extrae los vectores y sumamos a lo largo del eje del contexto (axis=1)
    h = cp.sum(W[batch_c_indices], axis=1) / C  # (B, N)

    # Producto matricial para todo el batch: (B, N) @ (N, V) = (B, V)
    u = h @ W_prima

    # Softmax por filas para el batch: shape (B, V)
...
def retropropagacion_batch(W, W_prima, batch_c_indices, batch_objetivos, h, y, eta):
    ...
    # 1. Error de salida: e = y - t (shape: B, V)
    e = y.copy()
...
def calcularPerdida_batch(y, batch_objetivos):
    """
    y: shape (B, V)
    batch_objetivos: shape (B,) con los índices de la palabra objetivo de cada ejemplo
    """
    B = y.shape[0]
...
def entrenar_por_batches(tokens, ventana, epocas, N, eta, batch_size=64):
    ...
    print(f"ENTRENANDO POR LOTES (Batch Size = {batch_size}) | Ventana = {ventana}")
    print(f"Tokens: {len(tokens)} | Vocabulario |V|: {V} | Ejemplos totales: {len(ejemplos)}")

    W, W_prima = inicializarPesos(V, N)

    # Convertimos los ejemplos a matrices de CuPy para eficiencia
    ...
    for epoca in range(1, epocas + 1):
        ...
        for i in range(0, num_ejemplos, batch_size):
            batch_idxs = indices_mezclados[i:i + batch_size]
            ...
            perdida_total += perdida * len(batch_idxs)
